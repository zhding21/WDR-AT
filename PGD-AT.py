...
from util import upper_limit, lower_limit, std, clamp, get_loaders, evaluate_pgd, evaluate_standard
...
def get_args():
    ...
    parser.add_argument('--out-dir', default='AT-PGD-ResNet18', type=str, help='Output directory')
...
def main():
    ...
    logfile = os.path.join(args.out_dir, 'output_AT-PGD.log')
    ...
    for epoch in range(args.epochs):
        ...
        logger.info('%d \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f', epoch, epoch_time - start_epoch_time, lr,
                    train_loss / train_n, train_acc / train_n, pgd_acc, test_acc)
    ...
    torch.save(best_state_dict, os.path.join(args.out_dir, 'model_PGD_Best.pth'))
    torch.save(model.state_dict(), os.path.join(args.out_dir, 'model_PGD_Final.pth'))
