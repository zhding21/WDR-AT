...
from utils.util import (upper_limit, lower_limit, std, clamp, get_loaders, attack_pgd, evaluate_pgd, evaluate_standard,
...
def main(model_val):
    ...
    output_path = os.path.join(args.out_dir, 'CIFAR-10-ResNet18', model_val, 'FAT')
    ...
    logging.basicConfig(
        ...
        datefmt='%Y/%m/%d %H:%M:%S',
        level=logging.INFO,
        filename=os.path.join(output_path, 'output_FAT_our.log'))
    logger.info(args)
    ...
    def eval_model(alp, bet):
        ...
        # model_path = os.path.join(args.out_dir, 'model_FAT')
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(train_loader, desc=f'{model_val}-{epoch+1}/{args.epochs}: train models') as data_iterator:
        ...
        torch.save(best_state_dict, os.path.join(output_path, f'model_FAT_Best_our_{start_time}.pth'))
        torch.save(model.state_dict(), os.path.join(output_path, f'model_FAT_Final_our_{start_time}.pth'))
        logger.info('Total train time: %.4f minutes', (train_time - start_train_time) / 60)

        # Evaluation
        model_test = MODEL[model_val](10).cuda()
...
if __name__ == "__main__":
    for model_val in ['PreActResNet18']:
        main(model_val)
