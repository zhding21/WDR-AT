...
parser.add_argument('--datasets', metavar='DATASETS', default='TinyImagenet', type=str,
...
parser.add_argument('--save-dir', dest='save_dir',
                    ...
                    default='results/CIFAR10/Sub-PGD/sub-at-our', type=str)     # 1685750104.9548244
parser.add_argument('--log-dir', dest='log_dir',
                    help='The directory used to save the log',
                    default='results/CIFAR10/Sub-PGD/sub-at-our', type=str)
parser.add_argument('--log-name', dest='log_name',
                    help='The log file name',
                    default='Sub-AT2.log', type=str)
...
parser.add_argument('--autoattack', '--aa', default=False, help='evaluate the model with AA (default: False)')
...
def main():
    ...
    logfile = os.path.join(args.log_dir, 'output_Sub-AT2.log')
    logging.basicConfig(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.save(model.state_dict(),
                   os.path.join(args.save_dir, 'train' + str(train_eps) + 'psgd_' + str(epoch) + '.pt'))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.attack == 'trades':
            ...
            input_adv = input_var + (
                        eps / 2 * torch.sign(torch.tensor([0.5]).cuda() - torch.rand_like(input_var).cuda()).cuda())
            input_adv = torch.clamp(input_adv, 0.0, 1.0)

            model.eval()
            input_adv = Guided_Attack(model, nn.CrossEntropyLoss(), input_adv, target, eps=eps, steps=1, P_out=P_out,
                                      l2_reg=10, alt=(i % 2))
