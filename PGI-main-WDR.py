...
def main(model_val):
    ...
    output_path = os.path.join(args.out_dir, 'CIFAR-10-ResNet18', model_val, 'PGI')
    ...
    logging.basicConfig(
        ...
        datefmt='%Y/%m/%d %H:%M:%S',
        level=logging.INFO,
        filename=os.path.join(output_path, 'output_PGI_our.log'))
    logger.info(args)
    ...
    def eval_model(alp, bet):
        ...
        for epoch in range(args.epochs):
            print(f'Model Architecture: {model_val}, {epoch + 1}/{args.epochs}, alp: {alp}; bet: {bet}')
            batch_size = args.batch_size
        ...
        torch.save(best_state_dict, os.path.join(output_path, f'model_PGI_Best_our_{start_time}.pth'))
        torch.save(model.state_dict(), os.path.join(output_path, f'model_PGI_Final_our_{start_time}.pth'))
