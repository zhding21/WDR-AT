...
def main():
    args = get_args()
    output_path = os.path.join(args.out_dir, 'CIFAR-10', 'FGSM')
    ...
    def eval_model(alp, bet):
        ...
        torch.save(best_state_dict, os.path.join(args.out_dir, 'model_best_our_' + str(start_time) + '.pth'))
        torch.save(model.state_dict(), os.path.join(args.out_dir, 'model_final_our_' + str(start_time) + '.pth'))
