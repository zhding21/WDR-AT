...
def main(model_val):
    ...
    output_path = os.path.join(args.out_dir, 'CIFAR-10-ResNet18', model_val, 'PGD')
    ...
    logging.basicConfig(
        ...
        datefmt='%Y/%m/%d %H:%M:%S',
        level=logging.INFO,
        filename=os.path.join(output_path, 'output_PGD_our.log'))
    logger.info(args)
    ...
    def eval_model(alp, bet):
        ...
        torch.save(best_state_dict, os.path.join(output_path, f'model_PGD_Best_our_{start_time}.pth'))
        torch.save(model.state_dict(), os.path.join(output_path, f'model_PGD_Final_our_{start_time}.pth'))
